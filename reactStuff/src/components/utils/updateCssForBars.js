export default function(player) {
  ...
  document.documentElement.style.setProperty('--exp-gradient', `linear-gradient(to left, gray ${Math.floor((player.expToNextLevel - player.exp) / player.expToNextLevel * 100) || 100}%, yellow ${Math.floor((player.expToNextLevel - player.exp) / player.expToNextLevel * 100) || 100}%)`);
};